...
- Коммит `48e664f` развернут на `185.65.200.69`: HTML-инъекция в сообщении
  устранена, добавлены honeypot и лимит 3 заявки/10 минут/IP.
- `complexmedia.service` active; публичные страницы, sitemap и безопасный
  honeypot-запрос API проверены с HTTP `200`.
